...
% plot error_rate or error_reinforce
...
for nSes = 1:nSessions
    ...
    for geno = 1:length(Genotypes)
        ...
        sem = nanstd(data.leftIncorrect(:,strcmp(genotype,Genotypes{geno})),0,2)/sqrt(sum(strcmp(genotype,Genotypes{geno})));
        plot(edges(1:end-1),y,'Color',colors{geno});
        errorshade(edges(1:end-1), y+sem, y-sem, colors{geno},0.3);
    end
    xlim([0 2])
    ...
    for geno = 1:length(Genotypes)
        ...
        sem = nanstd(data.rightCorrect(:,strcmp(genotype,Genotypes{geno})),0,2)/sqrt(sum(strcmp(genotype,Genotypes{geno})));
        plot(edges(1:end-1),y,'Color',colors{geno});
        errorshade(edges(1:end-1), y+sem, y-sem, colors{geno},0.3);
    end
    xlim([0 2])
    ...
    for geno = 1:length(Genotypes)
        ...
        sem = nanstd(data.rightIncorrect(:,strcmp(genotype,Genotypes{geno})),0,2)/sqrt(sum(strcmp(genotype,Genotypes{geno})));
